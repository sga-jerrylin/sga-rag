...
async def do_handle_task(task):
    ...
    if task_type == "memory":
        logging.warning("Legacy memory task %s is no longer supported and will be ignored.", task.get("id"))
        set_progress(task.get("id"), prog=-1, msg="[Unsupported]: legacy memory task is disabled.")
        return
